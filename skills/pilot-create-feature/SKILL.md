...
description: 自律開発ワークフローのFeature Issueを起票する。masterサブエージェントがroadmap先頭の未起票Featureを逐次起票する際や、pmがFeature分割・roadmap補充を行う際に必ずこのスキルを使用する。このスキルはESCALATEベースの自律実行専用であり、ユーザーが直接「Issueを作って」と依頼してくる対話的な起票にはcreate-issueを使うこと。
...
- ユーザ同席（ユーザが直接 `/pilot-create-feature` を呼び出した場合など、明示されたとき）: AskUserQuestionによる確認ができる
- 自律実行（masterからの逐次起票attach・pmサブエージェントからの分割起票など）: ユーザへの質問はできない。判断できない事項は `ESCALATE:` で呼び出し元に報告する
...
テンプレート `~/.claude/skills/pilot-setup/templates/spec-feature-issue.md.tmpl` に従って本文を作成し、起票する。
...
呼び出し元（master・pm・ユーザのいずれか）に以下を報告する。
