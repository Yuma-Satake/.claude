...
description: 自律開発ワークフローのFeature Issueを起票する。pilot-runがroadmap先頭の未起票Featureを逐次起票する際、またはpmがFeature分割やroadmap補充を行う際に使用する。人間との対話を前提とするcreate-issueと異なり、constitution・visionに基づく自己判断で起票し、判断できない事項はESCALATEで呼び出し元に報告する。
...
- 自律実行（pilot-runからの逐次起票dispatch・pmサブエージェントからの分割起票など）: ユーザへの質問はできない。判断できない事項は `ESCALATE:` で呼び出し元に報告する
