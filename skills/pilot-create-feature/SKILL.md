...
description: 自律開発ワークフローのFeature Issueを起票する。pilot-runがroadmap先頭の未起票Featureを逐次起票する際や、pmがFeature分割・roadmap補充を行う際に必ずこのスキルを使用する。このスキルはESCALATEベースの自律実行専用であり、ユーザーが直接「Issueを作って」と依頼してくる対話的な起票にはcreate-issueを使うこと。
...
- 自律実行（pilot-runからの逐次起票attach・pmサブエージェントからの分割起票など）: ユーザへの質問はできない。判断できない事項は `ESCALATE:` で呼び出し元に報告する
