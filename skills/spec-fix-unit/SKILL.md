---
name: spec-fix-unit
description: 自律開発ワークフローのUnit Issueのタスクを1件実装する。spec-orchestrateからdispatchされたworkerが、phase:implで次の未チェックタスクの実装、phase:impl_fixでreviewer指摘の修正を行う際にロードする。人間との対話を前提とするfix-issueと異なり、constitution照合による自己判断で進め、判断できない事項はESCALATEで報告する。
...
Unit Issue `#$0` のタスクを1件実装する。`~/.claude/rules/` 配下のルールおよびプロジェクト固有の `CLAUDE.md` に従って実装する。

## 前提

- 隔離されたworktree内で実行されている（呼び出し元が `isolation: "worktree"` を指定する）
- 最初に `git fetch origin` を実行する。リモートに `feature/issue-$0` が存在すればそれを追跡してswitchする（前のboltでpush済みの作業を引き継ぐため）。存在しなければ `origin/<デフォルトブランチ>` の最新から作成する

## 対象タスクの決定

- phase:impl の場合: Issue本文「タスク」セクションの上から最初の未チェックタスク1件のみを対象とする。複数のタスクを1回で実装しない
- phase:impl_fix の場合: Issueのjournalコメントにあるreviewer指摘への対応を対象とする
...
4. アーキテクチャ上の判断（データモデル・外部サービス選定・モジュール境界の変更等）を伴う場合、`docs/adr/` にADRを起案してコミットに含める
   - テンプレート: `~/.claude/skills/spec-bootstrap/templates/spec-adr.md.tmpl`
