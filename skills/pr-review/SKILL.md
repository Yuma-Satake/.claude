...
- **自分のPR**（PR番号指定時、authorが「現在の認証ユーザー」と一致する場合）、または **ローカル差分**（PR番号省略時）: 指摘一覧を報告した上で、**AskUserQuestion** で「指摘への修正・再レビューのサイクルを回すか、報告のみで終えるか」を確認する
  - 修正ループを回す場合:
    ...
    - 指摘に対応する → 再レビュー前に`package.json`・`CLAUDE.md`・`Makefile`等に既存定義されたフォーマッタ・リンタの自動修正コマンド（例: `npm run format`）があれば現在の作業ディレクトリ（5.で切り替えたworktreeがある場合はそのパス）で実行し、機械的に除去できる指摘を解消する。新しいツールは導入しない。対象は変更ファイルに絞り、コマンドがリポジトリ全体にしか適用できない場合は実行後に`git diff --name-only`で対象外のファイルが変更されていないか確認する → 該当skillのcode-reviewer agentを再度並列起動して再レビューを依頼する → 指摘がなくなるまで繰り返す。指摘内容のうち判断・意思決定が必要な事項は、推奨の対応が判断できる場合はその推奨で対応し、判断できない場合はAskUserQuestionで確認する
