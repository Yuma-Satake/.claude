...
- `Echo` インスタンスはサーバー起動後に変更しない（goroutine-safeでない）
...
- リクエストボディのバインドは `c.Bind(&req)` を使用し、バインド後に必ずバリデーションを実行する
...
- ハンドラーのテストは `httptest.NewRecorder()` と `httptest.NewRequest()` を使い、Echoインスタンスを使わずに単体でテストできる
