...
5. 対応内容を対象ごとに検証する。コード変更では関連する最小のテスト・ビルド・lintを実行する
6. `hunk session comment list <session-id> --type all --json` で既存注記を確認する
7. 対応と検証が完了した各ユーザーコメントについて、元コメントと同じファイル・同じ側・同じ開始行へ `hunk session comment add` で対応済み注記を追加する
8. 全コメントについて、対応済みまたは対応できなかった理由を最終報告する
...
実施した変更、検証結果、対応済み注記の追加結果を簡潔に報告する。対応できなかったコメントがある場合に限り、対象箇所と理由を明記する。コメントが0件の場合は変更を行わず、その旨を明示する。
