---
name: spec-fix-feature
description: 自律開発ワークフローのFeature Issueのタスクを1件実装する。spec-runからdispatchされたworkerが、phase:codingで次の未チェックタスクの実装、phase:wait_code_fixでreviewer指摘の修正を行う際にロードする。人間との対話を前提とするfix-issueと異なり、constitution照合による自己判断で進め、判断できない事項はESCALATEで報告する。
...
- phase:wait_code_fix の場合: Issueのjournalコメントにあるreviewer指摘への対応を対象とする
- タスクのチェックボックスは自分では更新しない（masterが検収後に更新する）
...
4. アーキテクチャ上の判断（データモデル・外部サービス選定・モジュール境界の変更等）を伴う場合、`docs/adr/` にADRを起案してコミットに含める
   - テンプレート: `~/.claude/skills/spec-setup/templates/spec-adr.md.tmpl`
...
呼び出し元（master）に以下を報告する。

- `COMPLETED_TASK:` 行: Issue本文のチェックボックスと完全一致するテキストを1行で記載する（masterがこの文字列で `- [ ]` → `- [x]` 置換を行うため、1文字の差異も許容されない）。例: `COMPLETED_TASK: Google Maps Platform の地図コンポーネントを実装する`
