...
reviewerが返す指摘件数は、reviewerの出力テキストから `ESCALATE:` を除くすべての指摘項目を masterが数える。reviewerが「指摘なし」と明記している場合のみ指摘 0 件とみなす。指摘の有無が判断できない場合は `blocked:human` を付与してユーザに確認を仰ぐ。
...
| reviewer指摘なし（spec_review） | phase:impl へ。並列枠に空きがあれば同 cycle 内で worker を dispatch する |
| reviewer指摘あり（spec_review） | reviewerの出力から指摘を全件抽出してjournalコメントに転記し phase:spec_fix へ。並列枠に空きがあれば同 cycle 内で pm を dispatch する |
...
| reviewer指摘あり（code_review） | reviewerの出力から指摘を全件抽出してjournalコメントに転記し phase:impl_fix へ。並列枠に空きがあれば同 cycle 内で worker を dispatch する |
