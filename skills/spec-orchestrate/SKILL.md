...
対象が0件で未起票Unitもない場合、全Unit完了か全件人間待ちかを判定して1行で報告し、boltを終了する。
...
報告を受けた後、masterは `docs/roadmap.md` の該当行のIssue列を `#NN` に書き換え、デフォルトブランチへ直接コミット&pushする（roadmap更新は共有リソースであり、masterに集約する）。`ESCALATE:` を受けた場合は起票せず、Unit名・質問内容をbolt終了報告に含めてユーザにmentionする。当該Unitは未起票のまま残り、裁定後の次boltで再試行する。
