...
description: 自律開発ワークフローの1bolt（1サイクル）を実行する。GitHub Issueのphaseラベルを見て、動かせるUnitにpm/worker/reviewerサブエージェントを振り分け、結果に応じてphaseを遷移させる。「boltを回して」「自律開発を進めて」と依頼された場合、または /loop で常駐駆動する場合に使用する。
---

# spec-orchestrate

自律開発ワークフローの1boltを実行する。`/loop 10m /spec-orchestrate` で常駐駆動する想定。
...
- 1boltで完結し、次のboltは/loopに任せる。boltをまたぐ情報はすべてjournalコメントに残す

## 前提条件

bolt開始時に確認し、欠けていたら `/spec-bootstrap` の実行が必要である旨を報告して終了する。/loop実行中はユーザがループを止めるまで同じ報告を繰り返すだけになるため、修復を試みない。

- `gh auth status` が通ること
- `docs/constitution.md` と `docs/vision.md` が存在すること
...
- 同一Issueへのdispatchは1boltに1回まで

対象が0件なら、全Unit完了か全件人間待ちかを判定して1行で報告し、boltを終了する。
...
- phase:spec_review → reviewer
  「docs/constitution.md と docs/vision.md をレビュー基準としてロードすること。観点: 非ゴールへの侵犯・優先順位の矛盾・受け入れ基準がテスト可能な形か・タスクがworker単独で実行できる粒度か。指摘を全件列挙すること」
- phase:spec_fix → pm
...
3. PRが既にある場合は状態を確認する。マージ済みならIssueのcloseを確認して完了。CI実行中なら何もせず次のboltで再確認する。CI失敗・コンフリクト・レビュー待ちで停滞している場合は `blocked:human` を付与し、状況をjournalコメントで報告する

### 6. bolt終了報告

動かしたUnit・遷移・エスカレーション有無を数行で報告して終了する。

## 制約

- 並列dispatchは最大3体（裁定回収のpmも枠に含める。phase:done処理は含めない）
