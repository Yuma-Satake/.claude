...
- phase:proposed → pm
  「docs/constitution.md と docs/vision.md を読むこと。Exploreで関連コードを調査すること。Issue本文の『受け入れ基準』と『タスク』を、本文の既存セクション構造を保って記入すること。テンプレート由来の説明文・プレースホルダは記入時に除去すること。記述粒度の制約: 受け入れ基準は1件1〜2行・簡潔な条件文のみ（検証手段の細部・判断根拠・設計経緯は書かない）、タスクは `- [ ] 動詞から始めるone-liner` 形式（目的・内容・依存等のサブフィールドは設けない）。説明・経緯・不採用事項はIssue本文に書かない。受け入れ基準が5個以上になる場合はUnit分割を検討し、分割する場合は spec-create-unit skill をロードして新Unitを起票し（自律実行であることを明示する）、本Issueのスコープを縮小し、着手順は依存セクションで強制すること。実装方針は書かないこと」
- phase:spec_review → reviewer
  「docs/constitution.md と docs/vision.md と docs/roadmap.md をレビュー基準としてロードすること。観点: Outcome自体のvision整合（独立してデプロイ可能か・単体で価値を持つか）・非ゴールへの侵犯・優先順位の矛盾・依存セクションの妥当性・受け入れ基準がテスト可能な形か・タスクがworker単独で実行できる粒度か。Outcomeに問題があれば受け入れ基準より先にOutcomeの修正を求めること。必須修正がある場合は `## 必須修正` セクションに列挙し、ない場合は `## 必須修正\nなし` と記載すること。出力の最終行は必ず `spec-decision: spec_fix`（必須修正ありの場合）または `spec-decision: impl`（必須修正なしの場合）の1行のみとすること」
- phase:spec_fix → pm
...
- phase:code_review → reviewer
  「`git fetch origin` を実行した上で、`origin/feature/issue-N` と `origin/<デフォルトブランチ>` のdiffをレビューすること。観点: Issue本文の受け入れ基準とテストコードの対応が取れているか・アーキテクチャ判断にADR（docs/adr/）が起案されているか・規約準拠。必須修正がある場合は `## 必須修正` セクションに列挙し、ない場合は `## 必須修正\nなし` と記載すること。出力の最終行は必ず `code-decision: impl_fix`（必須修正ありの場合）または `code-decision: impl`（必須修正なしの場合）の1行のみとすること」
- phase:impl_fix → worker（worktree隔離）
  「Skillツールで spec-fix-unit をロードし、Issue #N のjournalコメントにある `## 必須修正` の指摘のみを修正すること」
...
masterはreviewerの出力内容を独自に解釈しない。reviewer が出力の最終行に記載した `spec-decision` / `code-decision` を機械的に読んで遷移を決定する。これらのキーワードが見つからない場合は `blocked:human` を付与してユーザに確認を仰ぐ（reviewerが指示通りフォーマットで返さなかったことを示す）。

| 報告 | masterの処理 |
|---|---|
| `ESCALATE:` を含む | `blocked:human` を付与し、質問・主張をjournalコメントとして投稿する。phaseは変更しない。その後 Discord に通知する（後述） |
| pm完了（proposed / spec_fix） | 本文に受け入れ基準・タスクが記入されていることを `gh issue view N --json body` で確認し phase:spec_review へ。その後、並列枠に空きがあれば同 cycle 内で reviewer を dispatch する |
| `spec-decision: impl`（spec_review） | phase:impl へ。推奨・確認事項が出力にあっても無視する。並列枠に空きがあれば同 cycle 内で worker を dispatch する |
| `spec-decision: spec_fix`（spec_review） | reviewerの `## 必須修正` セクションのみをjournalコメントに転記し phase:spec_fix へ。並列枠に空きがあれば同 cycle 内で pm を dispatch する |
| spec-decision が見つからない（spec_review） | `blocked:human` を付与してユーザに確認を仰ぐ |
| worker完了（impl / impl_fix） | 実装したタスクのチェックボックス本文の転記と遷移記録を1つのjournalコメントにまとめて投稿し phase:code_review へ。チェックボックスはまだ更新しない。並列枠に空きがあれば同 cycle 内で reviewer を dispatch する |
| `code-decision: impl`（code_review） | チェックボックス本文の転記を含む最新のjournalコメントから対象タスクを特定しチェックボックスを更新する。未チェックタスクが残れば phase:impl へ、全完了なら phase:done へ |
| `code-decision: impl_fix`（code_review） | reviewerの `## 必須修正` セクションのみをjournalコメントに転記し phase:impl_fix へ。並列枠に空きがあれば同 cycle 内で worker を dispatch する |
