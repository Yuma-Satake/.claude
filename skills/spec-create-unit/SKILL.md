...
Unitの概要 `$ARGUMENTS` をもとに、自律開発ワークフロー用のUnit Issueを起票する。
...
Unitに関連するコード・ファイル・機能をExploreエージェントで軽く調査する（thoroughness: quick〜medium）。
...
3. 自律実行でconstitutionでも判断できない場合、不明点を本文の「関連コード・背景」に前提確認事項として明記する。Unitの成立自体に関わる不明点なら、起票後に結果の先頭へ `ESCALATE:` と質問を書いて呼び出し元に報告する（`blocked:human` の付与はmasterが行うため、自分では付けない）

### 3. 分割の判定

Unitが大きすぎる場合は分割する。基準は「独立してデプロイ可能かつ単体で価値を持つ」こと。

- 複数の独立したコンポーネントにまたがる変更が必要な場合
- 段階的にリリースできる区切りが明確に存在する場合

分割した場合、各Unitを個別に起票し、依存関係を「依存」セクションで結ぶ。後から起票されたUnitはIssue番号が大きくなり番号順 = 優先順位の前提が崩れるため、先に着手すべきUnitへの依存を必ず張って着手順を強制する。

### 4. Issueの起票

テンプレート `~/.claude/skills/spec-bootstrap/templates/spec-unit-issue.md.tmpl` に従って本文を作成し、起票する。

- タイトル: `[Unit] <名前>`（言い切り形で簡潔に）
...
```
gh issue create --title "[Unit] <名前>" --body-file <本文> --label "phase:proposed"
```
