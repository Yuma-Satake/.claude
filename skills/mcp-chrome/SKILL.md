...
個人情報の入力や本人確認書類のアップロードを伴うフォームで、確認画面への遷移・最終送信など後戻りできないボタンをクリックする前には、送信内容をテキストでユーザーに提示し、明示的な承認（「OK」など）を得てからクリックすること。

## 操作終了時のタブクリーンアップ

ユーザーの目的が達成され、ブラウザ操作が完全に終了するタイミングで、このセッションで開いたタブを `tabs_close_mcp` ですべてクローズすること。

**クローズするタイミング（ユーザーへの最終報告の直前）:**

- ユーザーが依頼したすべての操作が完了し、追加の作業が発生しないと判断できる時点
- ユーザーへ結果を報告して会話を締めくくる直前

**クローズしないタイミング（途中段階）:**

- 複数ステップの操作の途中（次のページへ遷移する前など）
- ユーザーから追加の指示が来る可能性がある段階
- 取得した値を確認してもらいながら進める段階

**手順:**

1. `tabs_context_mcp` でセッション中に開いたタブの ID を確認する
2. このセッションで新規作成したタブのみを `tabs_close_mcp` でクローズする（ユーザーが元から開いていたタブは閉じない）
3. クローズ後に最終報告を返す
