...
description: 自律開発ワークフローのFeature Issueのタスクを1件実装する。pilot-runからdispatchされたworkerが、phase:codingで次の未チェックタスクの実装、phase:wait_code_fixでreviewer指摘の修正を行う際にロードする。人間との対話を前提とするfix-issueと異なり、constitution照合による自己判断で進め、判断できない事項はESCALATEで報告する。
...
4. アーキテクチャ上の判断（データモデル・外部サービス選定・モジュール境界の変更等）を伴う場合、`docs/adr/` にADRを起案してコミットに含める
   - テンプレート: `~/.claude/skills/pilot-setup/templates/spec-adr.md.tmpl`
