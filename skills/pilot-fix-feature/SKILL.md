...
description: 自律開発ワークフローのFeature Issueのタスクを1件実装する。pilot-runからworkerとしてattachされる際（phase:coding・phase:wait_code_fix）に必ずロードする。このスキルは「実装フェーズのworker」専用であり、仕様作成・レビュー・Issue起票は行わない。ユーザーが直接「バグを修正して」と依頼する場合はfix-issueスキルを使うこと。
...
- phase:wait_code_fix の場合: Issueのpilot-logコメントにあるreviewer指摘への対応を対象とする
...
4. アーキテクチャ上の判断（データモデル・外部サービス選定・モジュール境界の変更等）を伴う場合、`docs/adr/` にADRを起案してコミットに含める
   - テンプレート: `~/.claude/skills/pilot-setup/templates/adr.md.tmpl`
