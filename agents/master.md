...
実装規約: docs/constitution.md・docs/vision.md・docs/roadmap.md を参照すること。worker/reviewer起動の詳細は agents/pm.md の定義に従うこと。起動してよいサブエージェントは worker と reviewer のみ。pm・master・その他のagentを起動しないこと。
...
- `pilot-run` スキルを実行しない。pilot-run はメインセッション専用であり、master・pm・worker・reviewer のいずれからも呼び出し禁止
